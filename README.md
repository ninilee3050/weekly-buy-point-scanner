...
매수포인트는 `MACD 하락영역에서 시작된 MACD 상승흐름`이 유지되는 동안 세 보조 조건이 처음 동시에 만족되는 주봉입니다. 관찰이 시작된 뒤에는 MACD가 상승영역으로 올라와도 MACD 하락흐름시작이 나오기 전까지 관찰을 유지합니다.
...
- 상승흐름시작 주에 MACD가 0 위로 올라왔더라도 직전 주가 하락영역이면 매수포인트 기록
...
11. MACD 상승흐름시작 주에 MACD가 0 위로 올라왔더라도 직전 주 MACD가 0 아래였으면 하락영역에서 시작된 상승흐름으로 인정합니다. 관찰 시작 후에는 MACD가 상승영역으로 올라가도 하락흐름시작 전까지 계속 관찰합니다.
12. Top 100 스캐너는 각 종목 데이터를 매번 새로 다운로드하고 기존 CSV를 덮어씁니다.
13. Top 100 스캐너 결과와 실패 종목은 `스캔 저장하기` 버튼을 눌렀을 때 사용자 다운로드 폴더에 날짜별 CSV로 저장합니다.
14. `weekly_scan.py`는 GitHub Actions 같은 자동실행 환경에서 쓰는 화면 없는 Top 100 스캐너입니다.
15. `weekly_scan.py` 결과와 실패 종목은 프로젝트 `outputs/` 폴더에 날짜별 CSV로 저장합니다.
16. 사용자가 “세이브포인트”라고 말하면 Git 커밋을 의미합니다.
17. 사용자가 “푸시”라고 말하면 로컬 커밋을 GitHub에 업로드하는 것을 의미합니다.
